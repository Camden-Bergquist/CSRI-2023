...
event_recorder <- function(tutorial_id, tutorial_version, user_id, event, data) {
  ...
  if (event %in% c("exercise_submission", "question_submission")) {
    ...
    new_row <- data.frame(
      ...
      question_num = paste0("Q", question_num),  # Add "Q" before the question number
      stringsAsFactors = FALSE  # Prevent strings being converted to factors
    )
    ...
    if(data$question == "[Question 5] Submit Assignment?") {
      
      # Reshape data frame to long format
      df_long <- df %>% 
        gather(key = "key", value = "value", -user_id, -question_num)
        
      print("df_long after gather:")
      print(df_long)
      ...
      print("df_wide after pivot_wider:")
      print(df_wide)
      
      # Generate the column order
      unique_questions <- sort(unique(df$question_num))
      col_order <- c("user_id", unlist(lapply(unique_questions, function(i) paste0(i, c("_problem", "_points", "_answer", "_correct")))))
...
```{r submission}
question("[Question 5] Submit Assignment?",
